...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(metric=metric, G=G, dataset_kwargs=args.dataset_kwargs,
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress, dataset2_kwargs=args.dataset2_kwargs if hasattr(args, 'dataset2_kwargs') else {}, cache = args.use_cache)
        if rank == 0:
...
@click.option('--use_cache', help='Use cached features', type=bool, default=True, metavar='BOOL', show_default=True)

def calc_metrics(ctx, network_pkl, metrics, data, data2, mirror, gpus, verbose, use_cache):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    # Validate arguments.
    args = dnnlib.EasyDict(metrics=metrics, num_gpus=gpus, network_pkl=network_pkl, verbose=verbose, use_cache = use_cache)
