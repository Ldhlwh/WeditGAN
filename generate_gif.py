...
@click.option('--num-cols', help='Number of columns', default=4, type=int)
...
def generate_gif(
    ...
    transfer = hasattr(G, 'transfer') and (G.transfer != 'none')
    ...
    def to_image_grid(outputs):
        ...
        outputs = np.concatenate(outputs, axis=1)
        return Image.fromarray(outputs).resize((resolution * num_cols, resolution * (num_rows if not transfer else 2 * num_rows)), Image.ANTIALIAS)
    ...
    def transfer_generate(dlatents):
        src_images = G.synthesis(dlatents, noise_mode = noise_mode, mod_intensity = 0.0)
        tgt_images = G.synthesis(dlatents, noise_mode=noise_mode)

        images = (torch.cat([src_images, tgt_images], dim = 2).permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8).cpu().numpy()
        return to_image_grid(images)
    ...
    for i in range(num_phases):
        ...
        for j in range(transition_frames):
            ...
            if transfer: 
                output_seq.append(transfer_generate(dlatents))
            else:
                output_seq.append(generate(dlatents))

        if transfer:
            output_seq.extend([transfer_generate(dlatents1)] * static_frames)
        else:
