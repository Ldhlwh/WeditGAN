...
def get_feature_detector(url, device=torch.device('cpu'), num_gpus=1, rank=0, verbose=False):
    if url is not None:
        ...
        return ros_inception.build_model(2048, device)
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, num_data = 1, **stats_kwargs):
    ...
    if opts.cache:
        ...
        cache_tag = f'{dataset.name}-{get_feature_detector_name(detector_url) if detector_url is not None else "torchvision-inception"}-{md5.hexdigest()}'
    ...
    progress = opts.progress.sub(tag=f'dataset {num_data} features', num_items=num_items, rel_lo=rel_lo, rel_hi=rel_hi)
    ...
    for images, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        ...
        if detector_url is None:
            ...
            features = ros_inception.get_activations(images, detector, device = opts.device)
        else:        
