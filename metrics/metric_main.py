...
def fid5k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=5000, use_torchvision_inception = True)
    return dict(fid5k_full=fid)

@register_metric
def kid5k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    kid = kernel_inception_distance.compute_kid(opts, max_real=1000000, num_gen=5000, num_subsets=100, max_subset_size=1000, use_torchvision_inception = True)
    return dict(kid5k_full=kid)

@register_metric
def fid_between_dir(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid_between_dir(opts, max_real=1000000, num_gen=5000, num_subsets=100, max_subset_size=1000, use_torchvision_inception = True)
    return dict(fid_between_dir=fid)

@register_metric
def kid_between_dir(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    kid = kernel_inception_distance.compute_kid_between_dir(opts, max_real=1000000, num_gen=5000, num_subsets=100, max_subset_size=1000, use_torchvision_inception = True)
    return dict(kid_between_dir=kid)

@register_metric
def lpips200(opts):
    lpips200 = lpips.compute_lpips(opts, num_gen = 200)
    return dict(lpips200 = lpips200)

@register_metric
def clpips1k(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    clpips1k, clpips1k_rz = lpips.compute_clpips(opts, num_gen = 1000, max_num_per_cluster = 50)
