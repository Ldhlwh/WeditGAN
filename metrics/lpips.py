...
def compute_clpips(opts, num_gen, max_num_per_cluster):
    dataset_kwargs = opts.dataset_kwargs
    device = opts.device
    G = copy.deepcopy(opts.G).eval().requires_grad_(False).to(device)

    with torch.no_grad():
        loss_fn_alex = lpips.LPIPS(net='vgg', verbose = opts.progress.verbose).to(device)
        ...
        for seed in iterator:
            ...
            if len(cluster[closest_index]) < max_num_per_cluster:
...
def compute_lpips(opts, num_gen):
    device = opts.device
    G = copy.deepcopy(opts.G).eval().requires_grad_(False).to(device)
    ...
    with torch.no_grad():
        ...
        for _ in range(img.shape[0] - 1):
            img = torch.cat([img[1:], img[0:1]], dim = 0)
            lpips_value += loss_fn_alex(img, ref_img).sum().item()
        lpips_value /= (img.shape[0] * (img.shape[0] - 1))
